...
BlockSize = 16
NumTables = 4
T_tables = {0: AES.T1, 1: AES.T2, 2: AES.T3, 3: AES.T4}
        ...
        for r in range(max(start_round, 1), end_round + 1):
            ...
            for i in range(NumTables):
                for l in range(int(BlockSize / NumTables)):
                    # TODO: Is this good for other key sizes?
                    accessed_list[j][i].append(stateround[i + l * NumTables] & 0xF0)
        ...
        for l in range(int(BlockSize / NumTables)):
            ...
            for j in range(num_traces):
                for nibble in unaccessed_list[j][i]:
                    # Since nibble wasn't accessed, we know that k[i + l * NumTables] != p[i + l * NumTables] ^ nibble
                    plain = plaintexts[j][i + l * NumTables]
                    guess_list[(plain ^ nibble) >> 4] = False
...
def calc_ttables(l, plaintexts, partial_k0_high):
    ...
    for j, plain in enumerate(plaintexts):
        for nibble in range(16):
            for i in range(NumTables):
                # TODO: check
                # computes the key index controlling this table access
                plain_index = (NumTables * l + (i * int(BlockSize / NumTables + 1))) & 0xf
                # key guess is the known upper nibble and some guess for lower nibble
                key_value = partial_k0_high[i] + nibble
                # plain text that affects this value is in same index as key
                plain_value = plain[plain_index]
                # access to table i is to the OTP of key and plaintext
                T_result[j][nibble][i] = T_tables[i][key_value ^ plain_value]
    return T_result
...
def find_unviable_candidates(j, T_result, k0_low, viable):
    ...
    # calculate the columns expected output if the lower nibbles are correct.
    col = T_result[j][k0_0][0] ^ T_result[j][k0_1][1] ^ T_result[j][k0_2][2] ^ T_result[j][k0_3][3]

    # split the column into the x^(2) bytes
    x1_0 = col >> 28 & 0xf
    x1_1 = col >> 20 & 0xf
    x1_2 = col >> 12 & 0xf
    x1_3 = col >> 4  & 0xf

    plains = [x1_0, x1_1, x1_2, x1_3]

    # Now we can play the same game as we played before on the first round, where x_1 is playing the part of plaintext.
    for cache_line in range(16):
        for i in range(NumTables):
            if not viable[cache_line][i]:
                unviable.append((plains[i] ^ cache_line, i))
...
def guess_k1_ttable(plaintexts, unaccessed_list, k0, verbose=False):
    ...
    # create a dummy second round key
    zeros = bytes.fromhex('00' * 16)
    combined = k0 + zeros
    aes = AESr(combined, 2) # AES will now have a 0 second round key and will do the encryption for us.

    r1 = np.empty(num_traces, dtype=bytearray)

    # TODO: fix
    # Given that we already know K0, we can fully recover the high parts of K1:
...
def check_test_vectors():
    ...
    unviable_candidates_set = set(find_unviable_candidates(0, T_result, 0, viable))
    expected_set = {(2, 0), (5, 0), (7, 0), (8, 0), (11, 0), (13, 0), (4, 1), (5, 1), (6, 1), (8, 1), (9, 1), (10, 1), (13, 1), (6, 2), (7, 2), (8, 2), (9, 2), (14, 2), (0, 3), (1, 3), (3, 3), (5, 3), (6, 3), (11, 3), (13, 3)}
    if unviable_candidates_set == expected_set:
        print("find_unviable_candidates: Functional")
    else:
        print("find_unviable_candidates: Not Functional")
        print(f'found {len(unviable_candidates_set)}, expected {len(expected_set)}')
        print(len(unviable_candidates_set.intersection(expected_set)))
        print(unviable_candidates_set.intersection(expected_set))
        print(unviable_candidates_set)
        return
...
def debug():
    key = '00112233445566778899aabbccddeeff101112131415161718191a1b1c1d1e1f'
    print(len(key))
    k0 = bytes.fromhex(key[:32])
    zeros = bytes.fromhex('00' * 16)
    combined = k0 + zeros
    aes = AESr(combined, 2)

    aes_orig = AESr(k0, 2)
    for k1,k2 in zip(aes._Ke[0], aes_orig._Ke[0]):
        if k1 != k2:
            print(f'ohoh {k1} != {k2}')
